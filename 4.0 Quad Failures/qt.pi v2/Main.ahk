...
class monitor{
	...
	usable(aVBor, aHBor, aVBorEx, aHBorEx, aTTask, aBTask, aLTask, aRTask, aTBRatio, aLRRatio){
		...
		this.tbSmallH := (this.UH - ((aTBRatio - 1) * aVBor)) / aTBRatio
		this.tbSmallW := (this.UW - (2 * aHBor)) / 3
		...
		this.lrSmallH := (this.UH - (2 * VHBor)) / 3
		this.lrSmallW := (this.UW - ((aLRRatio - 1) * aHBor)) / aLRRatio
...
#include Plugins\Fibonacci\Fibonacci.ahk
