	...
	reload(read = 0)
	{
		...
		if (read = 1)
		{
			FileReadLine, bspeed, config.txt, 121
			SetBatchLines, %batchspeed%
			
			FileReadLine, hres1, config.txt, 4
			FileReadLine, vres1, config.txt, 7
			
			FileReadLine, dis2, config.txt, 12
			if (dis2 = 1)
			{
				FileReadLine, hres2, config.txt, 15
				FileReadLine, vres2, config.txt, 18
				FileReadLine, off2, config.txt, 21
			}
			
			FileReadLine, dis3, config.txt, 26
			if (dis3 = 1)
			{
				FileReadLine, hres3, config.txt, 29
				FileReadLine, vres3, config.txt, 32
				FileReadLine, off3, config.txt, 35
			}
			
			FileReadLine, hbor, config.txt, 40
			FileReadLine, vbor, config.txt, 43
			FileReadLine, hborex, config.txt, 46
			FileReadLine, vborex, config.txt, 49
			
			FileReadLine, tbar1, config.txt, 54
			FileReadLine, tbar2, config.txt, 57
			FileReadLine, tbar3, config.txt, 60
			FileReadLine, bbar1, config.txt, 63
			FileReadLine, bbar2, config.txt, 66
			FileReadLine, bbar3, config.txt, 69
			FileReadLine, rbar1, config.txt, 72
			FileReadLine, rbar2, config.txt, 75
			FileReadLine, rbar3, config.txt, 78
			FileReadLine, lbar1, config.txt, 81
			FileReadLine, lbar2, config.txt, 84
			FileReadLine, lbar3, config.txt, 87
			
			FileReadLine, hspeed, config.txt, 93
			FileReadLine, vspeed, config.txt, 96
			
			FileReadLine, enablesound, config.txt, 102
			if (enablesound = 1)
			{
				FileReadLine, vol, config.txt, 105
				vold := vol * -1
			}	
			
			FileReadLine, enablebeep, config.txt, 110
			if (enablebeep = 1)
			{
				FileReadLine, freq, config.txt, 113
				FileReadLine, dura, config.txt, 116
			}
		}
